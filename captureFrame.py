...
def byterize_led_colors(led_colors):
    ...
    for colors in led_colors:
        for color in colors:
            byte_led_colors.append(bytes(color))
    return byte_led_colors


def concat_led_colors(led_colors):
    concat_led_colors_list = list()
    for colors in led_colors:
        for color in colors:
            concat_led_colors_list.append(color)
    return concat_led_colors_list
...
print("Wat we gaan sturen")
print(colors)
print("byterize")
concat_colors = concat_led_colors(colors)
byte_colors = np.array(concat_colors).astype(np.uint8)
print("dit")
print(byte_colors)
arduino.write(str(byterize_led_colors(colors)))
...
def main_loop():
    while (True):
        ...
        print("scherm")
        bytearray = led_colors.astype(np.uint8)
